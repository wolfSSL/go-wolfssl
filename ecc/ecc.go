...
func GenerateKey(curve int, rng []byte) (pub, priv []byte, err error) {
	...
	if ret := binding.Wc_InitRng(&wcrng); ret != 0 {
		return nil, nil, fmt.Errorf("failed to initialize RNG")
	}
	defer	binding.Wc_FreeRng(&wcrng)

	if ret := binding.Wc_ecc_init(&key); ret != 0 {
		return nil, nil, fmt.Errorf("failed to initialize ECC key")
	}
	defer	binding.Wc_ecc_free(&key)

	if ret := binding.Wc_ecc_make_key(&wcrng, 32, &key); ret != 0 {
	...
	if ret := binding.Wc_ecc_export_private_only(&key, priv, &privSz); ret != 0 {
