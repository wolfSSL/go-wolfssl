...
func main() {
    ...
    if alg == "sha" {
        ...
        wolfSSL.Wc_ShaHash(str, len(str), out)
    } else if alg == "sha256" {
        out = make([]byte, wolfSSL.WC_SHA256_DIGEST_SIZE )
        wolfSSL.Wc_Sha256Hash(str, len(str), out)
    } else if alg == "sha384" {
        out = make([]byte, wolfSSL.WC_SHA384_DIGEST_SIZE )
        wolfSSL.Wc_Sha384Hash(str, len(str), out)
    } else if alg == "sha512" {
        out = make([]byte, wolfSSL.WC_SHA512_DIGEST_SIZE )
        wolfSSL.Wc_Sha512Hash(str, len(str), out)
    } else if alg == "md5" {
        out = make([]byte, wolfSSL.WC_MD5_DIGEST_SIZE )
        wolfSSL.Wc_Md5Hash(str, len(str), out)
    } else {
